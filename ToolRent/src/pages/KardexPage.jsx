...
const KardexPage = () => {
  ...
  const handleFilter = async (filters) => {
    ...
    // VALIDATION: Check that end date is not before start date
    const startDate = new Date(filters.startDate);
    const endDate = new Date(filters.endDate);
    
    if (endDate < startDate) {
      alert("ERROR: La fecha final no puede ser anterior a la fecha inicial. Por favor verifica las fechas.");
      return;
    }
    ...
    try {
      ...
      // VALIDATION: If no movements found and a tool was selected, show alert
      if (data.length === 0 && filters.toolId) {
        alert("No se han registrado movimientos para la herramienta seleccionada en este periodo.");
      }
  ...
  return (
    <PageLayout tutorialData={tutorialContent.kardex}>
