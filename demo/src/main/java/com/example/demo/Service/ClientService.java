...
public class ClientService {
    ...
    public ClientEntity createClient(ClientEntity client) {
        // Rule 1: RUT is mandatory
        if (client.getRut() == null || client.getRut().isEmpty()) {
            throw new RuntimeException("Error: RUT is required.");
        }

        // Rule 2: Duplicate RUTs are not allowed
        if (clientRepo.findByRut(client.getRut()).isPresent()) {
            throw new RuntimeException("Error: A client with this RUT already exists.");
        }

        // Rule 3: Default status is Active (true)
    ...
    public ClientEntity updateClient(ClientEntity clientUpdates) {
        ...
        if (existingOptional.isPresent()) {
            ...
            throw new RuntimeException("Error: Client not found for update.");
        }
    }
    ...
    public ClientEntity processPayment(String rut, Integer amount) {
        ...
        ClientEntity client = clientRepo.findByRut(rut)
                .orElseThrow(() -> new RuntimeException("Error: Client not found with RUT: " + rut));

        // 2. Defensive Coding / Validation
        if (amount == null || amount <= 0) {
            throw new RuntimeException("Error: Payment amount must be positive.");
        }

        if (amount > client.getAmountClient()) {
            throw new RuntimeException("Error: Payment amount (" + amount +
                    ") cannot exceed current debt (" + client.getAmountClient() + ").");
        }
